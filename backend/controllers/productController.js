...
exports.getProductById = async (req, res) => {
  try {
    ...
    if (mongoose.Types.ObjectId.isValid(req.params.id)) {
      product = await Product.findById(req.params.id).populate('seller', 'name email phone storeName status');
    }
    
    if (!product) {
      product = await Product.findOne({ sku: req.params.id.toUpperCase() }).populate('seller', 'name email phone storeName status');
    }
