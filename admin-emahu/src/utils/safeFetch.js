...
/**
 * Universal safe fetch function for EMAHU Admin Panel.
  ...
  while (attempt <= retries) {
    try {
      ...
      if ((response.status === 503 || response.status === 502 || response.status === 504) && attempt < retries) {
        console.warn(`[admin safeFetch] Server waking up (${response.status}). Retrying attempt ${attempt + 1}/${retries}...`);
      ...
      if (contentType.includes('application/json')) {
        ...
        if (response.status === 404) {
          friendlyMessage = 'Admin API Endpoint not found (404). Please verify backend server deployment.';
      ...
      if (attempt < retries) {
        console.warn(`[admin safeFetch] Network error. Retrying attempt ${attempt + 1}/${retries}...`);
        await new Promise((res) => setTimeout(res, delayMs));
        attempt++;
        continue;
      }
